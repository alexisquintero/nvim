require('pluginsettings/lsp-config') -- Has lsp mappings fn

require('pluginsettings/packer')

require('pluginsettings/cmp')
require('pluginsettings/comment-nvim')
require('pluginsettings/fidget')
require('pluginsettings/flash')
require('pluginsettings/gitsigns')
require('pluginsettings/haskell-tools')
require('pluginsettings/ltex')
require('pluginsettings/luasnip')
require('pluginsettings/metals')
require('pluginsettings/netrw')
require('pluginsettings/rust-tools')
require('pluginsettings/sandwich')
require('pluginsettings/sexp')
require('pluginsettings/targets')
require('pluginsettings/telescope')
require('pluginsettings/treesitter')
require('pluginsettings/vim-cool')
require('pluginsettings/vim-printer')
